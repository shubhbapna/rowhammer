...
int main(int argc, char **argv) {
    ...
    for (i = 1; i < num_iterations; i++) {
        ...
        time = time / SAMPLES;
        if (ROW_BUFFER_CONFLICT_LATENCY_MIN < time < ROW_BUFFER_CONFLICT_LATENCY_MAX) {
            break;
        }
    }
    ...
    for (j = 1; j < num_iterations; j++) {
        ...
        uint64_t time = 0;
        for (int k = 0; k < SAMPLES; k++) {
            time += measure_bank_latency((uint64_t)base, (uint64_t)(base + j * ROW_SIZE));
        }
        time = time / SAMPLES;
        if (time < ROW_BUFFER_HIT_LATENCY) {
            time = 0;
            for (int k = 0; k < SAMPLES; k++) {
                time += measure_bank_latency((uint64_t)(base + i * ROW_SIZE), (uint64_t)(base + j * ROW_SIZE));
            }
            time = time / SAMPLES;
            if (ROW_BUFFER_CONFLICT_LATENCY_MIN < time < ROW_BUFFER_CONFLICT_LATENCY_MAX) {
                break;
