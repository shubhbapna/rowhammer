...
int main(int argc, char **argv) {
    ...
    setup_PPN_VPN_map(allocated_mem, mem_size);
    verified_results = fopen("verified.txt", "a");
    non_verified_results = fopen("non_verified.txt", "a");
    if (argc == 1) {
        while (true) {
            // not changing the if statement inside the function as it causes some weird branch prediction
            press_all();
        }
    } else {
